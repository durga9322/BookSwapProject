...
export default function Marketplace() {
  ...
  const [newBook, setNewBook] = useState({
    title: "", author: "", genre: "", condition: "Good", city: "", pincode: ""
  });
...
useEffect(() => { fetchBooks(); fetchMyBooks(); }, []);
  ...
  const addBook = async (e) => {
    ...
    try {
      ...
      setShowAdd(false);
      setNewBook({ title: "", author: "", genre: "", condition: "Good", city: "", pincode: "" });
      fetchBooks();
      fetchMyBooks();
      alert("Book added for swapping!");
    } catch (err) {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "2rem 1rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <h2 style={{ color: "#1e293b", fontSize: 24 }}>Book Swap Marketplace</h2>
        ...
        <form onSubmit={addBook} style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              ...
              <input required value={newBook.title} onChange={e => setNewBook({ ...newBook, title: e.target.value })} style={{ ...inputStyle, marginTop: 4 }} />
            </div>
            <div>
              <label style={{ fontSize: 12, color: "#374151" }}>Author</label>
              <input value={newBook.author} onChange={e => setNewBook({ ...newBook, author: e.target.value })} style={{ ...inputStyle, marginTop: 4 }} />
            </div>
            <div>
              <label style={{ fontSize: 12, color: "#374151" }}>Genre</label>
              <input value={newBook.genre} onChange={e => setNewBook({ ...newBook, genre: e.target.value })} style={{ ...inputStyle, marginTop: 4 }} />
            </div>
            <div>
            ...
            <div>
              ...
              <input value={newBook.city} onChange={e => setNewBook({ ...newBook, city: e.target.value })} style={{ ...inputStyle, marginTop: 4 }} />
            </div>
            <div>
              <label style={{ fontSize: 12, color: "#374151" }}>Pincode</label>
              <input value={newBook.pincode} onChange={e => setNewBook({ ...newBook, pincode: e.target.value })} style={{ ...inputStyle, marginTop: 4 }} />
            </div>
          </div>
      ...
      {loading ? (
        <p style={{ textAlign: "center", color: "#94a3b8", padding: "3rem" }}>Loading books...</p>
      ) : books.length === 0 ? (
        <div style={{ textAlign: "center", padding: "3rem", color: "#94a3b8" }}>
          <p style={{ fontSize: 18, marginBottom: 8 }}>No books available yet</p>
          <p style={{ fontSize: 14 }}>Be the first to add a book for swapping!</p>
        </div>
      ) : (
